...
class CoreSetsDBSCAN:
    ...
    p: The sample multiplier, which controls m, the number of points to sample
    ...
    def fit_predict(self, X, init="k-centers"):
        """
        ...
        X_tree = KDTree(X)
        radii = X_tree.query(X_sampled_pts, k=self.minPts)[0]
        ...
        closest = X_core_ind[closest_core_pts[:,0]]
